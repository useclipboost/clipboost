...
export default function ClipBoost() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Step 1: Submit the prompt layout request to the backend serverless file
      ...
      if (!response.ok || !data.success) {
        throw new Error(data.error || 'The server rejected the initial asset submission layout.');
      }

      // Grab the tracking reference ID returned instantly by your optimized code
      const renderId = data.renderId;
      if (!renderId) {
        throw new Error('Critical Error: Server did not return a tracking identifier.');
      }

      // Step 2: Kick off high-frequency frontend browser polling loop
      let completedUrl = null;
      let attempts = 0;
      const maxAttempts = 40; // 40 loops * 3 seconds = 120 seconds maximum render queue padding

      while (!completedUrl && attempts < maxAttempts) {
        setStatusMessage(`Rendering Pipeline Active... Cooking video frames (Check #${attempts + 1})`);
        
        // Pause execution for 3 seconds before requesting an update
        ...
        // Check if Shotstack's cloud system flipped the status flag to completed
        if (statusData.status === 'done' || statusData.status === 'completed') {
          completedUrl = statusData.url; // Capture the real, direct .mp4 streaming path
          break;
      ...
      // Success! Deliver direct video track directly into state
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', fontFamily: 'sans-serif', backgroundColor: '#0b0c10', color: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
        {/* Left Control Dashboard Section */}
        ...
        {/* Right Output Screen View Box Section */}
        <div style={{ backgroundColor: '#1f2833', padding: '30px', borderRadius: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px', relative: 'position' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', color: '#666' }}>
              <p style={{ margin: 0, fontSize: '15px' }}>Your rendered video preview will appear here inside a live streaming canvas layout layout container.</p>
            </div>
          )}
        </div>
      </div>
